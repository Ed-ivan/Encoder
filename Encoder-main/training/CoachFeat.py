'''
this  file is for train  feature_map
self.opts.train_encoder: Encoder which map image to 1 x 512
self.opts.train_featEncoder:which map feature
self.opts.train_decoder:which train them all
'''
'''
...
sys.path.append("/home/yqm/Encoder/Encoder-main")
import os
import matplotlib
import torchvision
import torch
import torch.nn as nn
from models.Eencoder import EEncoder
...
class FeatCoach(nn.Module):
    def configure_optimizers(self):
        ...
        if self.opts.train_decoder:
            assert self.opts.train_encoder==False,'When training all , opts.train_encoder should be false'
            assert self.opts.train_featEncoder == False, 'When training all , opts.train_featEncoder should be false'
            params = list(self.net.featEncoder.parameters())
            params += list(self.net.encoder.parameters())
    ...
    def __init__(self, opts):
        ...
        # Initialize network
        self.net = EEncoder(self.opts).to(self.device)
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                # loss
                if self.opts.use_full_segm:
                    full_segm = self.set_segm_dict(segm)["full_segm"]
                    loss, loss_dict = self.CaculateLoss(x, y_hat, full_segm)
                    loss.backward()
                    self.optimizer.step()
                    self.optimizer.zero_grad()
                # Logging related
    ...
    def set_segm_dict(self, segm):
        # return segm dict   暂时弃用  等想好 local的监督怎么添加再用
        segm_dict = {}
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                x = x.to(self.device).float()
                y_hat, latent = self.net.forward(x, return_latents=True)
            # compute moco loss  , segm loss , w regulazation
            loss, loss_dict = self.CaculateLoss(x, y_hat, segm)
            agg_loss_dict.append(loss_dict)
            self.parse_and_log_images(x, y_hat,
                                      title='images/test',
    ...
    def checkpoint_me(self, loss_dict, is_best):
        if self.opts.train_encoder:
            ...
            checkpoint_me_text="train_baseEncoder"
        ...
        if self.opts.train_featEncoder:
            ...
            checkpoint_path = os.path.join(self.checkpoint_dir, save_name)
            torch.save(save_dict, checkpoint_path)
            checkpoint_me_text="train_featEncoder"
        with open(os.path.join(self.checkpoint_dir, 'timestamp.txt'), 'a') as f:
            if is_best:
                f.write(
                    f'{checkpoint_me_text}**——Best**: Step - {self.global_step}, Loss - {self.best_val_loss} \n{loss_dict}\n')
                if self.opts.use_wandb:
                    self.wb_logger.log_best_model()
            else:
                f.write(f'{checkpoint_me_text}—Step - {self.global_step}, \n{loss_dict}\n')
    def CaculateLoss(self, y, y_hat, segm):
        '''
        l2  +style _loss + lpips
        :return:
        '''
        # 换一种方式进行 输出
        loss = 0.0
